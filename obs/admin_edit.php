					...
					<form method="post" action="admin_edit.php?bookisbn=<?php echo $row['book_isbn'];?>" enctype="multipart/form-data">
						...
						<div class="mb-3">
							...
							<select class="form-select rounded-0" name="publisherid" required>
								...
								while ($pub_row = mysqli_fetch_assoc($psql)):
								?>
								<option value="<?= $pub_row['publisherid'] ?>" <?= $row['publisherid'] == $pub_row['publisherid'] ? 'selected' : '' ?>><?= $pub_row['publisher_name'] ?></option>
								<?php endwhile; ?>
							</select>
						</div>
						<div class="mb-3">
							<label class="control-label">Quantity</label>
							<input class="form-control rounded-0" type="text" name="quantity" value="<?php echo $row['quantity'];?>" required>
						</div>
