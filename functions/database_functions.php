<?php
	...
	function updateBookQuantity($conn, $isbn, $newQuantity) {
		$query = "UPDATE books SET quantity = $newQuantity WHERE book_isbn = '$isbn'";
		return mysqli_query($conn, $query);
	}

	function getBookQuantityByIsbn($conn, $isbn) {
		$query = "SELECT quantity FROM books WHERE book_isbn = '$isbn'";
		$result = mysqli_query($conn, $query);
		if ($result) {
			$row = mysqli_fetch_assoc($result);
			if ($row && isset($row['quantity'])) {
				return $row['quantity'];
			}
		}
		return 0;
